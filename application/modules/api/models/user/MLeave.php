...
class mLeave extends CI_Model
{
    ...
    {
        $data = $this->db->insert('leave', $input);
        if (!empty($data)) {
            $response['status'] = TRUE;
            $id = $this->db->insert_id();
            $q = $this->db->get_where('leave', array('id' => $id));
            $response['data'] = $q->row();
        } else {
    ...
    {
        $data = $this->db->update('leave', $input, array('id' => $id));
    ...
    {
        $data = $this->db->delete('leave', array('id' => $id));
     ...
     /**
     * Get All request for approval Data from this method.
     *
     * @return Response
     */
    ...
    public function getSingleData($id)
    {
        if (!empty($id)) {
           // $data = $this->db->get_where("leave", ['id' => $id])->row_array();
            ...
            $response['error'] = 'id is a required parameter';
            return $response;
        }
        if (!empty($data)) {
            $response['status'] = TRUE;
