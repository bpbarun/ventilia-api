...
class Location extends REST_Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        $this->load->database();
        $this->load->model('user/mLocation');
    ...
    public function index_get($id = 0)
    {
        ...
        if (empty($responseData['error'])) {
            if (is_numeric($id)) {
                $response = $this->mLocation->getData($id);
            } else {
                $data = explode(":", $id);
                $response = $this->mLocation->getData($data);
            }
    ...
    public function index_post()
    {
        ...
        if (empty($responseData['error'])) {
            $input = $this->post();
            $response = $this->mLocation->insertData($input);
            $this->response($response, REST_Controller::HTTP_OK);
        } else {
            $this->response($responseData, REST_Controller::HTTP_UNAUTHORIZED);
        }
    ...
    public function index_put($id)
    {
        ...
        if (empty($responseData['error'])) {
            $input = $this->put();
            $response = $this->mLocation->updateData($id, $input);
            $this->response($response, REST_Controller::HTTP_OK);
        } else {
            $this->response($responseData, REST_Controller::HTTP_UNAUTHORIZED);
        }
    ...
    public function index_delete($id)
    {
        ...
        if (empty($responseData['error'])) {
            $input = $this->put();
            $response = $this->mLocation->deleteData($id);
            $this->response($response, REST_Controller::HTTP_OK);
        } else {
            $this->response($responseData, REST_Controller::HTTP_UNAUTHORIZED);
        }
    ...
    public function technicalsidebar_get($id)
    {
        ...
        if (empty($responseData['error'])) {
            ...
            $response = $this->mLocation->technicalsidebar($id);
